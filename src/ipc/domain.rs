...
impl IpcCommandWriteable for DomainMessageHeader {
    ...
    fn write(&self, buff: &mut [u32]) {
        let metadata = ((self.kind as u32) << 24)
            | ((self.object_id_count as u32) << 16)
            | (self.length as u32);
        buff[0] = metadata;
...
impl<T: IpcCommandReadable> IpcCommandReadable for DomainMessage<T> {
    fn word_count(&self) -> u32 {
        IpcCommandReadable::word_count(&self.header)
            + IpcCommandReadable::word_count(&self.payload)
            + self.header.object_id_count as u32
    }
    fn read(buff: &[u32]) -> Self {
        let header = DomainMessageHeader::read(buff);
        let payload = T::read(&buff[4..]);
        let object_id_start =
            IpcCommandReadable::word_count(&header) + IpcCommandReadable::word_count(&payload);
...
impl<T: IpcCommandWriteable> IpcCommandWriteable for DomainMessage<T> {
    fn word_count(&self) -> u32 {
        IpcCommandWriteable::word_count(&self.header)
            + IpcCommandWriteable::word_count(&self.payload)
            + self.header.object_id_count as u32
    }
    fn write(&self, buff: &mut [u32]) {
        self.header.write(buff);
        self.payload.write(&mut buff[4..]);
        let object_id_start = IpcCommandWriteable::word_count(&self.header)
            + IpcCommandWriteable::word_count(&self.payload);
        self.object_ids
            .as_ref()
            .write(&mut buff[object_id_start as usize..]);
    }
}
