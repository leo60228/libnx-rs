...
impl IpcSession {
    ...
    pub unsafe fn clone(&mut self, unknown: u32) -> Result<Self, LibnxError> {
        ...
        if resp.err_code != 0 {
            ...
            return Err(LibnxError::from_msg(format!(
                "Error while cloning {}: got no handles!",
                unknown
            )));
        } else {
    ...
    pub unsafe fn convert_to_domain(mut self) -> Result<u32, LibnxError> {
        ...
        let out_buf = slice::from_raw_parts(out_ptr, 16);
        let parsed =
            IpcCommandMessage::<ConvertSessionToDomainResponse>::parse_from_buffer(out_buf);
        let resp = parsed.payload();
        match resp.err_code {
            0 => Ok(resp.object_id),
    ...
    pub unsafe fn dispatch_command<T: IpcCommandWriteable>(
        &mut self,
        command: IpcCommandHeader<T>,
    ) -> Result<(), LibnxError> {
    ...
    pub unsafe fn close_object_by_id(&mut self, object_id: u32) -> Result<(), LibnxError> {
        ...
        let message: DomainMessage<EmptyArgs> =
            DomainMessage::with_header(message_header, EmptyArgs::new());
...
impl IpcCommandReadable for CloneSessionResponse {
    fn read(buff: &[u32]) -> Self {
        let magic = ((buff[1] as u64) << 24) | (buff[0] as u64);
        let err_code = ((buff[3] as u64) << 24) | (buff[2] as u64);
        CloneSessionResponse { magic, err_code }
    }
    fn word_count(&self) -> u32 {
        4
    }
...
impl Waitable for IpcSession {
    ...
    fn wait_synchronization(&self, timeout: Duration) -> Result<IpcSession, LibnxError> {
        ...
        let err_code =
            unsafe { waitHandles(&mut retval_idx as *mut i32, buffer_ptr, 1, timeout_nanos) };
        if err_code != 0 {
    ...
    T: AsRef<[IpcSession]>,
{
    ...
    fn wait_synchronization(&self, timeout: Duration) -> Result<IpcSession, LibnxError> {
        ...
        let err_code = unsafe {
            waitHandles(
                &mut retval_idx as *mut i32,
                buffer as *const _ as *const libnx::Handle,
                buffer_size as i32,
                timeout_secs,
            )
        };
